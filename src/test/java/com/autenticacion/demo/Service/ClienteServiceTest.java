...
class ClienteServiceTest {
    ...
    /**
     * Caso de prueba para el método (registrarCliente} en la clase
     * {ClienteService}.
     * 
     * This test verifies the following:
     * 
     * A new client is successfully registered using the provided
     * {ClienteRegistroDTO}.
     ...
     * Assert that the returned {ClienteRespuestaDTO} is not null and contains the
     * expected data.
    ...
    void testRegistrarCliente() {
        ...
        Cliente cliente = Cliente.builder()
                ...
                .rol(Rol.CLIENTE) // Debe coincidir con el hardcodeado en el método
                .build();

        when(passwordEncoder.encode(dto.getPassword())).thenReturn("encodedPassword");
        when(clienteRepository.save(any(Cliente.class))).thenReturn(cliente);

        // Mock para Kafka (nuevo)
        doNothing().when(kafkaProducer).enviarMensaje(anyString());
        ...
        assertEquals("Activo", respuesta.getEstadoCuenta());
        assertEquals(Rol.CLIENTE, respuesta.getRol()); // Verifica el rol hardcodeado

        // Verifica las interacciones
        verify(clienteRepository, times(1)).save(any(Cliente.class));
        verify(kafkaProducer, times(1)).enviarMensaje(anyString()); // Nueva verificación

        // Verifica que el password fue codificado
        verify(passwordEncoder, times(1)).encode(dto.getPassword());
    }

    /**
     * Método de prueba para
     * {@link ClienteService#obtenerIdClientePorEmail(String)}.
     * 
     ...
     * 
     * La prueba utiliza un repositorio simulado para simular la interacción con la
     * base de datos y
    ...
    /**
     * Caso de prueba para el método {@code actualizarCliente} de la clase
     * {@code ClienteService}.
     * 
     * Esta prueba verifica que el método {@code actualizarCliente} actualiza
     * correctamente la información de un cliente en el repositorio y devuelve un
     * booleano indicando el éxito de la operación.
     * información de un cliente en el repositorio y devuelve un booleano indicando
     * el éxito de la operación.
     * 
     * Pasos de la prueba:
     * 1. Definir un ID de cliente y un DTO que contenga la información actualizada
     * del cliente.
     * 2. Mock el método del repositorio {@code actualizarCliente} para que devuelva
     * un indicador de éxito (1).
     * 3. Llamar al método {@code actualizarCliente} en el servicio con el ID y el
     * DTO proporcionados.
     * 4. Comprobar que el método devuelve {@code true}, indicando que la
     * actualización se ha realizado correctamente.
     * 5. Verificar que el método del repositorio {@code actualizarCliente} fue
     * llamado exactamente una vez
     * con los parámetros correctos.
     * 
     * Comportamiento esperado:
     * - El método de servicio debería devolver {@code true} cuando el repositorio
     * indica una actualización exitosa.
     * - El método de repositorio debería ser invocado una vez con el ID de cliente
     * y el email correctos.
     */
    @Test
    void testActualizarCliente() {
        ...
        // Act
        boolean resultado = clienteService.actualizarCliente(id, dto);

        // Assert
        assertTrue(resultado);
        verify(clienteRepository).findById(id);
        verify(clienteRepository).save(clienteExistente);
        assertEquals("Nuevo Nombre", clienteExistente.getNombre());
        assertEquals("nuevo@email.com", clienteExistente.getEmail());
    }

    /**
     * Caso de prueba para el método {@code eliminarCliente} de la clase
     * {@code ClienteService}.
     * 
     * Esta prueba verifica que el método {@code eliminarCliente} delega
     * correctamente
     * la eliminación de un cliente al {@code clienteRepository} llamando a su
     * método
     * método {@code eliminarPorId} con el ID de cliente apropiado.
     * 
     * Pasos de la prueba:
     * 1. Simular el comportamiento de {@code clienteRepository.deleteById} para que
     * no haga nada.
    ...
    /**
     * Caso de prueba para el método `cambiarContraseña` en la clase
     * `ClienteService`.
     * 
     * Este test verifica lo siguiente:
     * 1. Se llama al método `findByEmail` de `clienteRepository` para recuperar la
     * entidad
     * entidad `Cliente` asociada al email proporcionado.
     * 2. El método `passwordEncoder.encode` se utiliza para codificar la nueva
     * contraseña.
     * 3. La contraseña de la entidad «Cliente» se actualiza con la contraseña
     * codificada.
     * 4. La entidad `Cliente` actualizada se guarda utilizando el método `save` de
     * `clienteRepository`.
     * 
     * Pasos de prueba:
     ...
     * - Se hacen aserciones para asegurar que la contraseña se actualiza
     * correctamente y se llama al método
